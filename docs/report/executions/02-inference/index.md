...
| :--- | :--- | :--- | :--- | :--- |
| M1 | requests served per second | `http_requests_total` on the Go API | **2026-09-02 BLOCKED** | required · the Go API exposes no `/metrics` endpoint at all (`GET /metrics` → 404) and neither `http_requests_total` nor any `http_*` series exists in Prometheus for it — confirmed twice, on two separate live runs. Not an instrumentation-gap-in-the-scrape-config problem like `01-ingestion`'s `kube_node_labels` was; the Go binary itself has no promhttp exporter wired up. Blocks M1-M3 entirely until `apps/api` is instrumented and a ServiceMonitor is added for `rag-api` |
| M2 | request duration distribution | `http_request_duration_seconds_bucket` | **2026-09-02 BLOCKED** | required · same root cause as M1 · histogram, p50 p95 p99 read from buckets, never averaged across the window · generation is stubbed, so this is a retrieval-path number and not an end-to-end SLO → K1 |
| M3 | error rate by status class | same family, status label | **2026-09-02 BLOCKED** | required · same root cause as M1 · a rate held at the cost of errors is not a sustained rate |
