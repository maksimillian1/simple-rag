...
| D22 | Bedrock VPC endpoint crossover, queries/month | `(3 ENI × $/ENI-h × 730) ÷ (NAT $/GB − PrivateLink $/GB)`, converted to queries at `(E18 input + output) × bytes-per-token`; `figures.yaml` → `endpoint_breakeven_queries` | blocked | the volume above which the PrivateLink interface endpoint costs less than the NAT gateway it replaces · **not resolvable yet**: the PrivateLink data-processing rate has never been pulled from the Price List API, so the figure reads `pending` · the range its assumptions produce, and why the decision does not wait on it, are in report §4.5 and K4 |
